...
import { Customer as ApiCustomer, UpdateCustomerRequest } from '@/types/api';
import { useUpdateCustomerMutation, useRoutes } from '@/hooks/use-queries';
...
export function EditCustomerModal({ isOpen, onClose, customer }: EditCustomerModalProps) {
  ...
  useEffect(() => {
    if (customer) {
      setFormData({
        ...
        routeId: typeof customer.route === 'object' ? customer.route._id : customer.route || '',
      });
    }
  }, [customer]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    updateCustomerMutation.mutate({ id: customer._id, data: formData }, {
      onSuccess: () => {
        toast.success('Customer updated successfully');
        handleClose();
      },
      onError: (error: unknown) => {
        const errorMessage = (error as { response?: { data?: { message?: string } } })?.response?.data?.message || 'Failed to update customer';
      ...
      <DialogContent className="w-full max-w-[90vw] sm:max-w-md mx-auto max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <DialogFooter className="flex flex-col-reverse sm:flex-row gap-2 sm:gap-0">
            ...
            <Button 
              type="submit" 
              disabled={updateCustomerMutation.isPending || routes.length === 0}
              className="w-full sm:w-auto"
            >
